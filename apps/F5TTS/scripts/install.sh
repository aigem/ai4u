...
# 提示先自行修改配置
echo "本脚本仅支持在 ${platform[name]} 平台运行"
...
if [ ! -d "${settings[venv_dir]}/${settings[venv_name]}" ]; then
    log_info "创建虚拟环境..."
    conda create -n ${settings[venv_name]} python=${settings[python_version]} -y
...
if ! conda activate ${settings[venv_name]}; then
    log_info "虚拟环境已激活"
fi
...
# 运行 F5-TTS 命令
run_cmd="f5-tts_infer-gradio --port ${settings[app_port]} --host 0.0.0.0"
...
echo "虚拟环境名称: ${settings[venv_name]}" >> "$APP_DIR/使用说明.md"
echo "如何启动虚拟环境: conda activate ${settings[venv_name]}" >> "$APP_DIR/使用说明.md"
echo "启动命令: $run_cmd" >> "$APP_DIR/使用说明.md"
echo "访问地址: http://<host>:${settings[app_port]}" >> "$APP_DIR/使用说明.md"
